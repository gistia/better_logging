  ...
  module RailsPlugins
    module BetterLogging
      ...
      def add_with_extra_info(severity, message = nil, progname = nil, &block)
        ...
        extra = ""
        if Thread.current[:extra_logging_info]
          extra = " [#{Thread.current[:extra_logging_info]}]"
        end

        message = "#{time}#{ActiveSupport::BufferedLogger.severity_name(severity)}#{extra} #{message}"
